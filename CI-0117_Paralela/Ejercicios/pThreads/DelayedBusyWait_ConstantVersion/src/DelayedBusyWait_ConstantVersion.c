...
typedef struct shared_data {
  ...
  uint64_t microsecs; //Var para microsegundos 
} shared_data_t;
...
int main(int argc, char *argv[]) {
  ...
  uint64_t microsecs = 0; //Var para microsegundos iniciailzar en 0
  if (argc == 3) { //Se espera que se ingresen 3 argumentos
    if (sscanf(argv[1], "%" SCNu64, &thread_count) == 1) {
    } else {
      fprintf(stderr, "Error: invalid thread count\n");
      return 11;
    }
    // se busca la cantidad de microsegundos ingresada
    if (sscanf(argv[2], "%" SCNu64, &microsecs) == 1) {
    } else {
      fprintf(stderr, "Error: invalid microsecs\n");
      return 11;
    }
  }
...
if (shared_data) {
  ...
  shared_data->microsecs = microsecs; //Se asigna el valor de microsegundos al atributo de la estructura
