...
Este programa en C utiliza **hilos (`pthread`)** para ejecutar múltiples tareas en paralelo. Su funcionamiento principal es:  

1. **Determinar la cantidad de hilos**:  
   - Si el usuario proporciona un argumento, lo usa como el número de hilos.  
   - Si no, usa la cantidad de núcleos del procesador.  

2. **Generar un número aleatorio** entre 0 y 100 para determinar el tiempo de espera (`randomSecs`) de los hilos.  

3. **Crear múltiples hilos**:  
   - Cada hilo espera su turno antes de imprimir un mensaje.  
   - El orden de ejecución está sincronizado mediante `next_thread`, asegurando que los hilos impriman en secuencia.  

4. **Medir y mostrar el tiempo de ejecución** usando `clock_gettime()`.  

El programa está diseñado para ilustrar la creación, sincronización y ejecución de múltiples hilos en C, asegurando que cada uno espere su turno antes de ejecutarse.

### Gráfica
 
---
![Gráfica Comparativa](Gráfica%20Comparativa.PNG)