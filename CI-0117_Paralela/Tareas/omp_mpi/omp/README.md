...
El simulador trabaja a partir de archivos de entrada binarios (denominados job), que contienen la configuración inicial de la placa. Utiliza OpenMP (Open Multi-Processing) para realizar los cálculos de manera paralela mediante directivas de compilación, aprovechando eficientemente los múltiples núcleos de procesamiento en sistemas de memoria compartida.
...
3. Ejecute el simulador con la siguiente sintaxis:  
   ```bash
   ./bin/omp tests/job002b/job002.txt 2
   ```
   Donde:
